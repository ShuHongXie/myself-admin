...
import { useLocalStorageWithExpire } from './your-hooks-path' // 复用之前的带过期存储Hooks
...
// 自动保存草稿（每10秒保存一次，7天过期）
const { hasChange, lastSaveTime, restoreDraft, clearDraft } = useFormAutoSave(
  'long_form_draft',
  formData.value,
  10,
  60 * 60 * 24 * 7
...
<template>
  <div class="long-form">
    ...
    <textarea v-model="formData.detail" placeholder="详细信息"></textarea>

    <!-- 草稿状态提示 -->
    <div v-if="hasChange" style="color: orange">有未保存的修改</div>
    <div v-else-if="lastSaveTime">最后保存：{{ lastSaveTime }}</div>

    <!-- 草稿操作 -->
    <button @click="restoreDraft">恢复草稿</button>
