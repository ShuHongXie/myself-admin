...
2. 将某个包安装进多个子包：pnpm add lodash --recursive --filter=@minilo/web --filter=@minilo/locales --save-catalog
3. 从多个子包中删除某个包：pnpm remove lodash --recursive --filter=@minilo/web --filter=@minilo/locales
4. 更新项目下的所有 typescript 包：pnpm up --recursive typescript@latest
5. 下载本项目的某个子包，并安装到当前项目：pnpm add @包名/子包名@workspace，例如：pnpm add @minilo/component@workspace
...
5. yalc link：负责 “将快照链接到本地项目”（开发维度，无需改版本）。
6. yalc remove：负责 “删除本地快照”（版本维度）。
7. 修改代码 → 构建（pnpm build） → yalc push（快速同步），效率更高。当需要标记正式版本时，再执行 yalc publish。
